...
def get_queries_from_multi(
    ...
    indices_2_filename = os.path.join(
        data_dir, f"indices_2_num{num_query}_q{len_query}"
    )
    ...
    if not os.path.exists(indices_1_filename) or not os.path.exists(indices_2_filename):
        ...
        with open(indices_1_filename, "wb") as fp, open(
            indices_2_filename, "wb"
        ) as gp, open(label_dummy_filename, "wb") as hp:
...
def load_queries_with_indices(
    ...
    if balance:
        ...
        for key, val in batch.items():
            batch[key] = val[np.concatenate([selected_zero_idx, nonzero_idx])]
        print(f"size of batch after balancing: {len(batch['labels'])}")
    print(f"size of batch: {len(batch['labels'])}")
    return batch
...
def qlearning_robosuite_dataset(dataset_path, terminate_on_end=False, **kwargs):
    ...
    for ep in tqdm(demos, desc="load robosuite demonstrations"):
        ...
        for i in range(traj_len - 1):
            ...
            obs = np.concatenate(
                [total_obs[key][i].tolist() for key in obs_keys], axis=0
            )
            new_obs = np.concatenate(
                [total_obs[key][i + 1].tolist() for key in obs_keys], axis=0
            )
