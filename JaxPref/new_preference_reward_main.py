...
def main(_):
    ...
    if FLAGS.robosuite:
        dataset = r_tf.qlearning_robosuite_dataset(
            os.path.join(
                FLAGS.robosuite_dataset_path,
                FLAGS.env.lower(),
                FLAGS.robosuite_dataset_type,
                "low_dim.hdf5",
            )
        )
        env = EnvUtils.create_env_from_metadata(
            env_meta=dataset["env_meta"], render=False, render_offscreen=False
        ).env
    ...
    if os.path.exists(base_path):
        ...
        with open(
            os.path.join(base_path, human_indices_1_file), "rb"
        ) as fp:  # Unpickling
            human_indices = pickle.load(fp)
        with open(
            os.path.join(base_path, human_indices_2_file), "rb"
        ) as fp:  # Unpickling
        ...
        human_indices_2_file, human_indices_1_file, script_labels_file = sorted(
            os.listdir(base_path)
        )
        with open(
            os.path.join(base_path, human_indices_1_file), "rb"
        ...
        with open(
            ...
            human_indices_2 = pickle.load(fp)
        with open(
            os.path.join(base_path, script_labels_file), "rb"
        ) as fp:  # Unpickling
            human_labels = pickle.load(fp)
        true_eval = True if len(human_labels) > FLAGS.num_query else False
        pref_eval_dataset = r_tf.load_queries_with_indices(
            ...
            window=FLAGS.window,
            feedback_random=FLAGS.feedback_random,
            pref_attn_n_head=FLAGS.transformer.pref_attn_n_head,
            true_eval=true_eval,
        )
    ...
    for epoch in range(FLAGS.n_epochs + 1):
            ...
            for i in range(interval):
                ...
                with Timer() as train_timer:
                    ...
                    for key, val in prefix_metrics(
                        reward_model.train(batch, env), "reward"
                    ).items():
        ...
        if epoch % FLAGS.eval_period == 0:
            for j in range(eval_interval):
                ...
                for key, val in prefix_metrics(
                    reward_model.evaluation(batch_eval, env), "reward"
                ).items():
                    metrics[key].append(val)
            if not criteria_key:
                if (
                    "antmaze" in FLAGS.env
                    and not "dense" in FLAGS.env
                    and not true_eval
                ):
