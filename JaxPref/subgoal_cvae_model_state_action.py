...
class ConditionalEncoder(nn.Module):
    ...
    @nn.compact
    def __call__(self, state, action, subgoal_state,subgoal_action):
        x = jnp.concatenate([subgoal_state,subgoal_action, state, action], axis=-1)
...
class SubgoalCVAE(nn.Module):
    ...
    def setup(self):
        ...
        self.decoder = Decoder(self.state_dim + self.action_dim, self.hidden_dims[::-1])

    def __call__(self, state, action, subgoal_state=None, subgoal_action=None, training=True):
        ...
        if training and subgoal_state is not None:
            ...
            post_mu, post_log_var = self.encoder(state, action, subgoal_state,subgoal_action)
