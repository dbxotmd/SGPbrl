...
class MetaworldDataset(Dataset):
    def __init__(self, dataset, clip_to_eps: bool = True, eps: float = 1e-3):
...
def metaworld_reward_from_preference(
    env_name: str,
    ...
    for i in trange(interval):
        start_pt = i * batch_size
        end_pt = min((i + 1) * batch_size, data_size)  # 데이터 크기 초과 방지
...
def metaworld_reward_from_preference_transformer(
    env_name: str,
    dataset: MetaworldDataset,
    reward_model,
    seq_len: int,
    ...
    use_diff: bool = False,
    label_mode: str = "last",
    with_attn_weights: bool = True,  # Option for attention analysis.
):
    # Get dimensions
    N, L = dataset.observations.shape[:2]
    ...
    # Create trajectories
    trajectories = []
    total_trajectories = N  # doubled due to stacking

    # Split the stacked data into individual trajectories
    observations = dataset.observations
    actions = dataset.actions

    # Create trajectory list
    for i in range(total_trajectories):
        trajectories.append((observations[i], actions[i]))

    # Create trj_mapper
    trj_mapper = []
    for trj_idx in range(total_trajectories):
        for seg_idx in range(L - 1):  # L-1 because of the next_observations structure
            trj_mapper.append((trj_idx, seg_idx))
